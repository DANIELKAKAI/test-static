...
export default class Jobs extends Component{
  ...
  componentWillMount(){
    fetch(END_POINT+ 'jobs',{
      method:'post',
    }).then(res => res.json()).then((result)=>{
        if(result.status===1){
          this.setState({jobs:result.jobs.reverse()})
        }
    },
    (error)=>{
    ...
    const { jobs, currentPage, JobsPerPage } = this.state;
        ...
        const renderPageNumbers = pageNumbers.map(number => {
         ...
         <div className="job-post-item bg-white p-4 d-block d-md-flex align-items-center">
            ...
            <div className="mb-4 mb-md-0 mr-2">
             <div className="job-post-item-header d-flex align-items-center">
               <h2 className="mr-3 text-black h4">{job.title}</h2>
               
             </div>
             <div className="job-post-item-body d-md-flex d-block">

                <p>
                <Markup content={job.description.slice(0,150)} />
                </p>
              </div>
		...
		return(
				...
				<section className="ftco-section bg-light" ref={this.myRef}>
